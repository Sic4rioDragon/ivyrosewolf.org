...
function parseStateValue(raw, category, section) {
  if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
    return {
      unlocked: !!raw.unlocked,
      progress: Number(raw.progress ?? (raw.unlocked ? 1 : 0)),
      goal: Number(raw.goal ?? 1),
      category,
      section
    };
  }
  ...
  for (const [topKey, topValue] of Object.entries(grouped || {})) {
    ...
    for (const [sectionKey, sectionValue] of Object.entries(topValue)) {
      ...
      for (const [achievementId, rawValue] of Object.entries(sectionValue)) {
        const category = topKey === 'survivors'
          ? 'survivor'
          : topKey === 'killers'
            ? 'killer'
            : 'general';

        out.set(achievementId, parseStateValue(rawValue, category, sectionKey));
      }
    }
  }
...
function renderAchievementCard(item) {
  ...
  const pct = item.goal > 0 ? Math.max(0, Math.min(100, (item.progress / item.goal) * 100)) : 0;
...
async function init() {
  ...
  state.items = base.map(item => {
    ...
    return {
      ...
      progress: Number(entry.progress ?? (entry.unlocked ? 1 : 0)),
      goal: Number(entry.goal ?? 1)
    };
  });
